 ...
 * This class implements the BuyLootBoxInputBoundary interface and is
 * responsible for buying loot box.
...
public class BuyLootBoxInteractor implements BuyLootBoxInputBoundary {
     ...
     */
    public BuyLootBoxInteractor(final BuyLootBoxOutputBoundary output,
                                final BuyLootBoxDataAccessInterface dataAccessObj) {
     ...
     */
    @Override
    public void execute(final BuyLootBoxInputData inputData) {
        // Get current user
        ...
        // buy loot box
        // This will: 1) consume coins, 2) generate random pet,
        // 3) add pet to inventory 4) name the pet
        user.buyLootBox(lootBox);

        // get the newly generated pet (last pet in inventory)
        Pet newPet = user.getPetInventory().get(
                user.getPetInventory().size() - 1);
